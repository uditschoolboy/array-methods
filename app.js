const inventors = [
  ...
  const inventors1500 = inventors.filter(({year}) => year >= 1500 && year < 1600);
  ...
  const sorted = inventors.sort((inventor1, inventor2) => inventor2.year - inventor1.year);
  ...
  const tally = data.reduce((obj, vehicle) => {
    if(!obj[vehicle]) {
        obj[vehicle] = 0;
    }
    obj[vehicle]++;
